...
\vspace{-1em}
\begin{itemize}
\item Los capítulos no se numeran
...
\begin{frame}[fragile]{Construcción de la Tapa}
La construcción de la Tapa en \LaTeX{} requiere un poco de trabajo y uso de la sintaxis y comandos, pero veremos que no es difícil de aprender.

Vamos a construir un ejemplo de tapa para una Tesis, en la cual incluiremos:
\begin{itemize}
\item Título de la Tesis
\item Autor, Director
\item Universidad y Facultad en la cual se desarrolla la Tesis
\item Fecha
\end{itemize}
...
En primer lugar, utilizaremos el entorno \textbf{titlepage} para generarla.

Luego, utilizaremos el comando \textbf{newgeometry} para modificar únicamente la geometría de la tapa.

Con el comando \textbf{vspace} generamos un espacio vertical en blanco de la longitud que deseemos.

Con el comando \textbf{rule} dibujamos una línea horizontal del ancho que deseemos.

Luego, con el entorno \textbf{minipage} podemos dividir la página verticalmente en columnas.



\end{frame}

\begin{frame}[fragile]{Tapa - Ejemplo}
...
\vspace{1cm}
\textsc{\Large Facultad de Ciencias Exactas, Físicas
        y Naturales}
\vspace{1cm}
...
\begin{frame}{Tapa - Templates}

Algunos templates de tapas:

\url{https://www.overleaf.com/gallery/tagged/title-page/}
\end{frame}
